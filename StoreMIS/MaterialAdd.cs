...
namespace StoreMIS
{
	...
	/// MaterialAdd 的摘要说明。
	/// </summary>
	public class MaterialAdd : System.Windows.Forms.Form
	{
		...
		{
			if (textID.Text.Trim()=="")
				MessageBox.Show("请输入物资编号！","提示");
			else
			{	
				oleConnection1.Open();
				string sql = "select * from materialinfo where MID = '"+textID.Text.Trim()+"'";
				this.oleCommand1.CommandText = sql;

				if (null == oleCommand1.ExecuteScalar())
				{
					string sql1 = "insert into materialinfo values ('"+textID.Text.Trim()+"','"+textName.Text.Trim()+"',"+
						"'"+textModel.Text.Trim()+"','"+textType.Text.Trim()+"','"+textUnit.Text.Trim()+"')";
					oleCommand1.CommandText=sql1;
					oleCommand1.ExecuteNonQuery();
					MessageBox.Show("添加物资信息成功！","提示");
				}
				else
					MessageBox.Show("物资编号"+textID.Text.Trim()+"已经存在！","警告");
